    ...
    <div class="row">
        <div class="col-xl-9">
            <form class="row g-3 mb-6 needs-validation" novalidate="" method="POST"
                action="{{ route('st_sp.update', $stSp->id) }}" onsubmit="showLoader()">
                @csrf
                @method('PUT')
                <div class="col-sm-12 col-md-6">
                    <div class="form-floating">
                        <input class="form-control @error('date') is-invalid @enderror" id="date" type="date"
                            name="date" placeholder="Tanggal" value="{{ old('date', $stSp->date) }}" required />
                    ...
                    <div class="form-floating">
                        <select class="form-select @error('status') is-invalid @enderror" id="status" name="status"
                            ...
                            @foreach (['Open', 'Close'] as $status)
                                <option value="{{ $status }}"
                                    {{ old('status', $stSp->status) == $status ? 'selected' : '' }}>
                                    {{ $status }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        @error('status')
                            <div ```blade class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                </div>
                        ...
                        <select class="form-select @error('employee_id') is-invalid @enderror" id="employee_id"
                            ...
                            @foreach ($employees as $employee)
                                <option value="{{ $employee->id }}"
                                    {{ old('employee_id', $stSp->employee_id) == $employee->id ? 'selected' : '' }}>
                                    {{ $employee->name }}
                                </option>
                            @endforeach
                        </select>
