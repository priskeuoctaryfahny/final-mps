    ...
    <div class="row">
        <div class="col-xl-8">
            <form class="row g-3 mb-6 needs-validation" novalidate="" method="POST"
                action="{{ route('gases.update', $gas->id) }}" onsubmit="showLoader()">
                @csrf
                @method('PUT')
                <div class="col-sm-12 col-md-12">
                    <div class="form-floating">
                        <input class="form-control" id="number" type="text" name="name"
                            placeholder="Nama/Tipe Gas" value="{{ $gas->name }}" required />
                    ...
                    <div class="form-floating form-floating-advance-select">
                        ...
                        <select class="form-select" id="user_id" data-choices="data-choices" size="1"
                            ...
                            @foreach ($users as $user)
                                <option value="{{ $gas->user_id ?? $user->id }}"
                                    {{ $gas->user_id == $user->id ? 'selected' : '' }}>
